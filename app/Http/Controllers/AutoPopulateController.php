...
class AutoPopulateController extends Controller
{
     ...
     */
    public function manualPopulate(Request $request)
    {
        $case = $request->case; // null; // 

        switch ($case) {
            case 'zcurrent':
            // dd(\App\Region::whereSlug(str_slug('Arad'))->first()->id);
                $region = 'Vrancea';
                ...
                foreach ($cities as $city) {
                    \App\City::create([
                        'name'    => $city,
                        'slug'    => str_slug($city),
                        'region_id' => \App\Region::whereSlug(str_slug($region))->first()->id,
                    ]);
                }
                echo $region .' # Done!.';
                break;
